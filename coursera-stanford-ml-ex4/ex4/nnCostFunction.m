...
%         computed in ex4.m
    ...
    % converting y to a (m,num_labels) to translate the actual number to a proper node response
    outputMapper = eye(num_labels);
    theY = zeros(m, num_labels);
    for i=1:m
      theY(i, :)= outputMapper(y(i), :);
    end

    % the inner sum sums by rows
    regularization = (lambda/(2*m)) * ( sum( sum(Theta1(:, 2:end).^2, 2) ) + sum( sum(Theta2(:,2:end).^2, 2) ));

    %sum by rows first *hence the ,2 then sum all
    J = 1/m * sum( sum((-theY .* log(Hx)) - ((1-theY) .* log(1-Hx)),2));
    J = J + regularization;
